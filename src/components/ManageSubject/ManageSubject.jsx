...
function ManageSubject() {
  ...
  const fetchSubjects = async () => {
    try {
      const response = await axios.get("http://localhost:3000/getSubject");
  ...
  const handleDeleteSubject = async () => {
    ...
    try {
      await axios.delete(`http://localhost:3000/deleteSubject/${selectedSubject.subject_id}`);
      const updatedSubjects = subjects.filter(subject => subject.subject_id !== selectedSubject.subject_id);
      setSubjects(updatedSubjects);

      // ถ้าหลังจากลบแล้วไม่มีข้อมูลในหน้าปัจจุบัน → ย้อนกลับไปหน้าก่อนหน้า
      const remainingItemsOnPage = updatedSubjects.slice(indexOfFirstSubject, indexOfLastSubject).length;
                ...
                <tr key={subject.subject_id}>
                  ...
                  <td>
                    <div className="subject-button-container">
                      <button className="edit-button" onClick={() => navigate(`/EditSubjectAdmin/${subject.subject_id}`)}>
                        <i className="bx bx-edit"></i> แก้ไข
                      </button>
                      <button className="delete-button" onClick={() => openDeletePopup(subject)}>
      ...
      {showDeletePopup && (
        <div className="delete-popup-overlay">
          <div className="delete-popup">
            ...
            <p>ชื่อวิชา: <strong>{selectedSubject?.subject_thai}</strong></p>
            <div className="popup-buttons">
              <button className="cancel-popup-button" onClick={closeDeletePopup}>ยกเลิก</button>
              <button className="confirm-button" onClick={handleDeleteSubject}>ยืนยัน</button>
            </div>
          </div>
        </div>
      )}
